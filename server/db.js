...
function ensureDemoAccount() {
  ...
  db.prepare('UPDATE users SET name = ? WHERE lower(trim(email)) = lower(?)').run('Demo User', DEMO_EMAIL);
  try {
    db.prepare('UPDATE users SET name = ? WHERE lower(trim(name)) = lower(?)').run('Demo User', 'Demo Investor');
  } catch {
    /* ignore */
  }
